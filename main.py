...
# ==========================================
def analyze_with_gemini(raw_text, ah_table=""):
    try:
...
N. HH:MM ทีมเหย้า พบ ทีมเยือน (ชื่อลีก)
🎯 <คำแนะนำ: เยือนไม่แพ้ / บ้านไม่แพ้ / เสมอ / หาผู้ชนะ> เส้นจริงจากตาราง (ถ้ามี)
⭐ X ดาว (YY%)
📌 เหตุผลสั้น 1-2 บรรทัด

⏰ เวลา HH:MM: เอาจากตารางราคาจริงเท่านั้น (เวลาแข่งล้วน ห้ามใส่วันที่) · ถ้าคู่นั้นแข่งอยู่/บอลสด ให้บอกสถานะตาม Forebet · ไม่มีเวลาในข้อมูล = เว้นว่าง ห้ามเขียน "[เวลา]" หรือ placeholder ใดๆ
👥 ชื่อทีม: เอาชื่อจริงทั้ง 2 ทีมจากตาราง/ข้อมูลเสมอ (แปลเป็นไทย) · 🚫 ห้ามเขียน "ทีมเยือน"/"เจ้าบ้าน"/ชื่อลอยๆ แทนทีมจริง · หาชื่อครบ 2 ทีมไม่ได้ = ตัดคู่นั้นทิ้ง
...
ข้อมูลดิบ (หลายตลาดรวมกัน — ใช้ประกอบเหตุผล/ดาว · แต่เส้น HDP+เวลา ยึดตารางด้านบนเท่านั้น):
{raw_text}
"""
        last_err = None
...
def main():
    ...
    print(f"📊 ตารางราคาจริง: {max(0, len(ah_table.splitlines()) - 1)} คู่" if ah_table else "⚠️ ไม่มีตารางราคา AH (Gemini จะไม่ใส่เส้น)")
