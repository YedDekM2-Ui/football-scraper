...
def analyze_with_gemini(raw_text):
    try:
        ...
        # นำกฎและเงื่อนไขทั้งหมดมาใส่ไว้ใน Prompt ตรงนี้ครับ
        prompt = f"""
คุณคือ Football Live Analyst หน้าที่ของคุณคือดึงข้อมูลจากข้อความด้านล่างนี้มาสรุปให้เข้าใจง่ายที่สุดตามเงื่อนไขนี้:

1. การอ่านสถานะเกม:
- ตรวจจับสถานะ: 'เกมหยุด', 'เลื่อน', 'จบ' และแสดงผลสกอร์สดพร้อมเวลาปัจจุบันใต้ชื่อคู่เสมอ
- นำผลที่ Forebet คาดการณ์มาแสดงไว้ท้ายสุด (เพื่อให้ผู้ใช้กรองต่อเอง)

2. รูปแบบคำแนะนำการแทง (ใช้คำเหล่านี้เท่านั้น):
- 'เยือนไม่แพ้', 'บ้านไม่แพ้', 'เสมอ', 'หาผู้ชนะ'
- พร้อมระบุ HDP/Over ให้ชัดเจนตามเดิม

3. การคัดกรองและเรียงลำดับ:
- ใช้เกณฑ์ดาว: 4 ดาว (80-99%), 3.5 ดาว (65-79%), 3 ดาว (50-64%)
- เรียงลำดับ 4 ดาวไว้บนสุดเสมอ
- ถ้าเป็น 'เลื่อน' หรือ 'เกมหยุด' ให้ขึ้นเตือนตัวหนา: ⚠️ [สถานะพิเศษ: บอลเลื่อน/หยุด]

4. การจัดการช่วงเวลา:
- ยึดกรอบ 10:01 น. ของวันนี้ ถึง 10:00 น. ของวันถัดไปเป็น 'บอลวันนี้'
- ดึง 'TOP Predictions' ของวันที่เรียกดูมาสรุปเสริมในกลุ่ม 'บอลวันนี้'

5. คำสั่งเปิด/ปิด:
- แสดงปุ่ม [🟢 เปิดใช้งาน] และ [🔴 ปิดแจ้งเตือน] ไว้ที่ท้ายข้อความสรุปทุกครั้ง
