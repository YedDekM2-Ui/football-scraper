...
🔴 การเรียงลำดับ (สำคัญสุด ทำก่อนอย่างอื่น): แบ่งเป็น 2 กลุ่มแล้ววางตามนี้เสมอ —
   ...
   ภายในแต่ละกลุ่มค่อยเรียงคู่ที่มั่นใจมากสุด (คู่ที่ได้เหรียญ 🥇🥈🥉 → ดาวเยอะ) ไว้บน · รวมสองกลุ่มไม่เกิน {MAX_MATCHES} คู่

1. สถานะเกม: 🚫 **ห้ามเขียนบรรทัด "สกอร์สด: X - Y" เด็ดขาด** — ระบบเติมบรรทัด LIVE:นาที  X - Y (X - Y) ให้เองอัตโนมัติใต้ชื่อคู่ ถ้าคุณเขียนอีกจะกลายเป็นซ้ำ 2 บรรทัด · ให้เอาสกอร์สดไปใช้ "คิด" อย่างเดียว แล้วพูดถึงในบรรทัด 📌 ได้ · ถ้าเลื่อน/หยุด ค่อยขึ้นเตือนตัวหนา: ⚠️ **[บอลเลื่อน/หยุด]**
...
• ยิงฝั่งเดียว = BTTS ฝั่ง No ≥55% + สกอร์คาดมีฝั่งยิง 0 แต่ห้ามเป็น 0-0 (ต้อง 1-0 / 2-0 / 3-0) + มีฝั่งต่อขาด (เส้น ≥ -1.5 = เต็งกินรวบ) หรือ avg goals ≤2.3 · ⚠️ ถ้าเกมส่อจืดยิงกันไม่ออกทั้งคู่ (สกอร์คาด 0-0) ห้ามฟันตัวนี้ ให้ใช้ 'ต่ำเต็ม' แทน
...
🔴 กฎสกอร์สด "ตามได้" (ใช้กับ **คู่บอลสด/เตะไปแล้วเท่านั้น** — หัวใจของการล็อก): อ่าน 3 ค่าพร้อมกัน = (1) เวลาที่เตะไปแล้ว (2) % ของตลาด (3) สกอร์สดตอนนั้น แล้วตัดสินว่า "คำทายกำลังจะเป็นจริงไหม":
   • ✅ สกอร์สด **นำไปทางที่ Forebet ทายแล้ว** (เช่นทายเยือนชนะ→เยือนนำอยู่ · ทายต่ำ→ยังยิงกันน้อย) = "ตามได้" → **ยิ่งเตะไปเยอะ + % ยิ่งสูง = ยิ่งพลิกยาก = ดันขึ้นบนสุดของกลุ่มบอลสด และมีสิทธิ์ได้เหรียญ 🥇**
...
จากนั้นแต่ละคู่ ใส่เลขลำดับ 1. 2. 3. (เรียง: **บอลสด/เตะไปแล้ว ไว้บนสุดก่อน → แล้วบอลวันนี้ที่ยังไม่เตะ** · ในแต่ละกลุ่มเรียงดาวมากสุดไว้บน · เหรียญ 🥇🥈🥉 แปะให้ 3 คู่ที่มั่นใจสุดของทั้งรายการ) รูปแบบ:
...
🎯 <คำแนะนำ: เยือนไม่แพ้ / บ้านไม่แพ้ / เสมอ / หาผู้ชนะ / สูงแรก / สูงเต็ม / ต่ำแรก / ต่ำเต็ม / ช่วยกันยิงหรือจบ2+ / ยิงฝั่งเดียว> + เส้นตัวเลขเท่านั้น (เช่น 'เยือนไม่แพ้ +0.25' · 'สูงเต็ม 2.5' · BTTS ไม่ต้องมีเลข)
...
🔒 นิ่ง N รอบ   **หรือ**   🔀 เพิ่งเปลี่ยน (เดิม: <pick เดิม>)   ← เลือกเขียนอย่างใดอย่างหนึ่งเท่านั้น ห้ามเขียนทั้งสองอันในบรรทัดเดียว ห้ามมีเครื่องหมาย / คั่น
   (ดูจาก "ประวัติทีเด็ดรอบก่อนๆ" · pick รอบนี้ตรงของเดิม→เขียน 🔒 นิ่ง เดิม+1 รอบ อย่างเดียว · ต่างจากเดิม→เขียน 🔀 เพิ่งเปลี่ยน (เดิม: ...) อย่างเดียว · คู่ที่ไม่มีในประวัติ/รอบแรกของวัน = **เว้นบรรทัดนี้ไปเลย** ไม่ต้องเขียน)
...
def build_from_api(matches, leagues, time_map, flag_map, odds_map, mkt_map):
    ...
    for mid, r in matches.items():
        ...
        sc = ""
        if r.get("Host_SC") not in (None, ""):
            sc = f" | สกอร์ {r.get('Host_SC')}-{r.get('Guest_SC')}"
            if r.get("Host_SC_HT") not in (None, ""):
                sc += f" (HT {r.get('Host_SC_HT')}-{r.get('Guest_SC_HT')})"
